...
then
  ...
  then
    ...
    echo "Do an intial ssh to each node from $NODE_NAME1"
    ...
    echo "Do an intial ssh to each node from $NODE_NAME1" 
    for i in $(seq 1 $TOTAL_NODES); do
      vagrant ssh -c "echo uptime|ssh -o StrictHostKeyChecking=no ${PUB_NET}.22${i}" "$NODE_NAME1"
    done
